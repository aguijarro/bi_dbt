...
\ pset tuples_only on -- Support to auto-generate UUIDs (aka GUIDs)
...
-------------------------------------
CREATE SCHEMA Person CREATE TABLE BusinessEntity(
    BusinessEntityID SERIAL,
    --  NOT FOR REPLICATION
    rowguid uuid NOT NULL CONSTRAINT "DF_BusinessEntity_rowguid" DEFAULT (uuid_generate_v1()),
    -- ROWGUIDCOL
    ...
    -- XML("AdditionalContactInfoSchemaCollection"),
    Demographics XML NULL,
    -- XML("IndividualSurveySchemaCollection"),
    ...
    CONSTRAINT "CK_Person_EmailPromotion" CHECK (
        EmailPromotion BETWEEN 0 AND 2
    ),
    CONSTRAINT "CK_Person_PersonType" CHECK (
        PersonType IS NULL
    ...
    rowguid uuid NOT NULL CONSTRAINT "DF_StateProvince_rowguid" DEFAULT (uuid_generate_v1()),
    -- ROWGUIDCOL
    ...
    Name "Name" NOT NULL,
    rowguid uuid NOT NULL CONSTRAINT "DF_AddressType_rowguid" DEFAULT (uuid_generate_v1()),
    -- ROWGUIDCOL
    ...
    rowguid uuid NOT NULL CONSTRAINT "DF_BusinessEntityAddress_rowguid" DEFAULT (uuid_generate_v1()),
    -- ROWGUIDCOL
    ...
    rowguid uuid NOT NULL CONSTRAINT "DF_BusinessEntityContact_rowguid" DEFAULT (uuid_generate_v1()),
    -- ROWGUIDCOL
    ...
    rowguid uuid NOT NULL CONSTRAINT "DF_EmailAddress_rowguid" DEFAULT (uuid_generate_v1()),
    -- ROWGUIDCOL
...
\ copy Person.BusinessEntity
FROM './BusinessEntity.csv' DELIMITER E'\t' CSV;
SELECT 'Copying data into Person.Person';
\ copy Person.Person
FROM './Person.csv' DELIMITER E'\t' CSV;
SELECT 'Copying data into Person.StateProvince';
\ copy Person.StateProvince
FROM './StateProvince.csv' DELIMITER E'\t' CSV;
SELECT 'Copying data into Person.Address';
\ copy Person.Address
FROM './Address.csv' DELIMITER E'\t' CSV ENCODING 'latin1';
SELECT 'Copying data into Person.AddressType';
\ copy Person.AddressType
FROM './AddressType.csv' DELIMITER E'\t' CSV;
SELECT 'Copying data into Person.BusinessEntityAddress';
\ copy Person.BusinessEntityAddress
FROM './BusinessEntityAddress.csv' DELIMITER E'\t' CSV;
SELECT 'Copying data into Person.ContactType';
\ copy Person.ContactType
FROM './ContactType.csv' DELIMITER E'\t' CSV;
SELECT 'Copying data into Person.BusinessEntityContact';
\ copy Person.BusinessEntityContact
FROM './BusinessEntityContact.csv' DELIMITER E'\t' CSV;
SELECT 'Copying data into Person.EmailAddress';
\ copy Person.EmailAddress
FROM './EmailAddress.csv' DELIMITER E'\t' CSV;
SELECT 'Copying data into Person.Password';
\ copy Person.Password
FROM './Password.csv' DELIMITER E'\t' CSV;
SELECT 'Copying data into Person.PhoneNumberType';
\ copy Person.PhoneNumberType
FROM './PhoneNumberType.csv' DELIMITER E'\t' CSV;
SELECT 'Copying data into Person.PersonPhone';
\ copy Person.PersonPhone
FROM './PersonPhone.csv' DELIMITER E'\t' CSV;
SELECT 'Copying data into Person.CountryRegion';
\ copy Person.CountryRegion
FROM './CountryRegion.csv' DELIMITER E'\t' CSV;
CREATE SCHEMA HumanResources CREATE TABLE Department(
    ...
    Org varchar NULL,
    -- hierarchyid, will become OrganizationNode
    OrganizationLevel INT NULL,
    -- AS OrganizationNode.GetLevel(),
    ...
    CONSTRAINT "CK_Employee_BirthDate" CHECK (
        BirthDate BETWEEN '1930-01-01' AND NOW() - INTERVAL '18 years'
    ),
    CONSTRAINT "CK_Employee_MaritalStatus" CHECK (UPPER(MaritalStatus) IN ('M', 'S')),
    -- Married or Single
    CONSTRAINT "CK_Employee_HireDate" CHECK (
        HireDate BETWEEN '1996-07-01' AND NOW() + INTERVAL '1 day'
    ),
    CONSTRAINT "CK_Employee_Gender" CHECK (UPPER(Gender) IN ('M', 'F')),
    -- Male or Female
    CONSTRAINT "CK_Employee_VacationHours" CHECK (
        VacationHours BETWEEN -40 AND 240
    ),
    CONSTRAINT "CK_Employee_SickLeaveHours" CHECK (
    ...
    CONSTRAINT "CK_EmployeeDepartmentHistory_EndDate" CHECK (
        (EndDate >= StartDate)
        OR (EndDate IS NULL)
    )
    ...
    -- 1 = monthly salary, 2 = biweekly salary
    CONSTRAINT "CK_EmployeePayHistory_Rate" CHECK (
        Rate BETWEEN 6.50 AND 200.00
    )
...
\ copy HumanResources.Department
FROM './Department.csv' DELIMITER E'\t' CSV;
SELECT 'Copying data into HumanResources.Employee';
\ copy HumanResources.Employee
FROM './Employee.csv' DELIMITER E'\t' CSV;
SELECT 'Copying data into HumanResources.EmployeeDepartmentHistory';
\ copy HumanResources.EmployeeDepartmentHistory
FROM './EmployeeDepartmentHistory.csv' DELIMITER E'\t' CSV;
SELECT 'Copying data into HumanResources.EmployeePayHistory';
\ copy HumanResources.EmployeePayHistory
FROM './EmployeePayHistory.csv' DELIMITER E'\t' CSV;
SELECT 'Copying data into HumanResources.JobCandidate';
\ copy HumanResources.JobCandidate
FROM './JobCandidate.csv' DELIMITER E'\t' CSV ENCODING 'latin1';
SELECT 'Copying data into HumanResources.Shift';
\ copy HumanResources.Shift
FROM './Shift.csv' DELIMITER E'\t' CSV;
...
DECLARE got_none BOOLEAN;
BEGIN LOOP got_none := true;
-- 01 = 0-3
UPDATE HumanResources.Employee
SET organizationnode = organizationnode || SUBSTRING(org, 3, 2)::bit(2)::INTEGER::VARCHAR || CASE
        SUBSTRING(org, 5, 1)
        WHEN '0' THEN '.'
        ELSE '/'
    END,
...
WHERE org LIKE '01%';
IF FOUND THEN got_none := false;
END IF;
-- 100 = 4-7
UPDATE HumanResources.Employee
SET organizationnode = organizationnode || (SUBSTRING(org, 4, 2)::bit(2)::INTEGER + 4)::VARCHAR || CASE
        SUBSTRING(org, 6, 1)
        WHEN '0' THEN '.'
        ELSE '/'
    END,
...
-- 101 = 8-15
UPDATE HumanResources.Employee
SET organizationnode = organizationnode || (SUBSTRING(org, 4, 3)::bit(3)::INTEGER + 8)::VARCHAR || CASE
        SUBSTRING(org, 7, 1)
        WHEN '0' THEN '.'
        ELSE '/'
    END,
...
SET organizationnode = organizationnode || (
        (
            SUBSTRING(org, 4, 2) || SUBSTRING(org, 7, 1) || SUBSTRING(org, 9, 3)
        )::bit(6)::INTEGER + 16
    )::VARCHAR || CASE
        SUBSTRING(org, 12, 1)
        WHEN '0' THEN '.'
...
SET organizationnode = organizationnode || (
        (
            SUBSTRING(org, 5, 3) || SUBSTRING(org, 9, 3) || SUBSTRING(org, 13, 1) || SUBSTRING(org, 15, 3)
        )::bit(10)::INTEGER + 80
    )::VARCHAR || CASE
        SUBSTRING(org, 18, 1)
        WHEN '0' THEN '.'
...
CREATE SCHEMA Production CREATE TABLE BillOfMaterials(
    ...
    CONSTRAINT "CK_BillOfMaterials_EndDate" CHECK (
        (EndDate > StartDate)
        OR (EndDate IS NULL)
    ),
    ...
    Doc varchar NULL,
    -- hierarchyid, will become DocumentNode
    ...
    CONSTRAINT "CK_Document_Status" CHECK (
    ...
    -- int
    Name "Name" NOT NULL,
    rowguid uuid NOT NULL CONSTRAINT "DF_ProductCategory_rowguid" DEFAULT (uuid_generate_v1()),
    -- ROWGUIDCOL
    ...
    Name "Name" NOT NULL,
    rowguid uuid NOT NULL CONSTRAINT "DF_ProductSubcategory_rowguid" DEFAULT (uuid_generate_v1()),
    -- ROWGUIDCOL
    ...
    -- XML(Production.ProductDescriptionSchemaCollection)
    Instructions XML NULL,
    -- XML(Production.ManuInstructionsSchemaCollection)
    rowguid uuid NOT NULL CONSTRAINT "DF_ProductModel_rowguid" DEFAULT (uuid_generate_v1()),
    -- ROWGUIDCOL
    ...
    CONSTRAINT "CK_Product_ProductLine" CHECK (
        UPPER(ProductLine) IN ('S', 'T', 'M', 'R')
        OR ProductLine IS NULL
    ),
    CONSTRAINT "CK_Product_Class" CHECK (
        UPPER(Class) IN ('L', 'M', 'H')
        OR Class IS NULL
    ),
    CONSTRAINT "CK_Product_Style" CHECK (
        UPPER(Style) IN ('W', 'M', 'U')
        OR Style IS NULL
    ),
    CONSTRAINT "CK_Product_SellEndDate" CHECK (
    ...
    CONSTRAINT "CK_ProductCostHistory_EndDate" CHECK (
        (EndDate >= StartDate)
        OR (EndDate IS NULL)
    ),
    ...
    rowguid uuid NOT NULL CONSTRAINT "DF_ProductDescription_rowguid" DEFAULT (uuid_generate_v1()),
    -- ROWGUIDCOL
    ...
    -- smallint
    Name "Name" NOT NULL,
    CostRate numeric NOT NULL CONSTRAINT "DF_Location_CostRate" DEFAULT (0.00),
    -- smallmoney -- money
    ...
    rowguid uuid NOT NULL CONSTRAINT "DF_ProductInventory_rowguid" DEFAULT (uuid_generate_v1()),
    -- ROWGUIDCOL
    ...
    CONSTRAINT "CK_ProductInventory_Bin" CHECK (
        Bin BETWEEN 0 AND 100
    )
    ...
    CONSTRAINT "CK_ProductListPriceHistory_EndDate" CHECK (
        (EndDate >= StartDate)
        OR (EndDate IS NULL)
    ),
    ...
    CONSTRAINT "CK_ProductReview_Rating" CHECK (
    ...
    StockedQty INT,
    -- AS ISNULL(OrderQty - ScrappedQty, 0),
    ...
    CONSTRAINT "CK_WorkOrder_EndDate" CHECK (
        (EndDate >= StartDate)
        OR (EndDate IS NULL)
    )
    ...
    CONSTRAINT "CK_WorkOrderRouting_ActualEndDate" CHECK (
...
\ copy Production.BillOfMaterials
FROM 'BillOfMaterials.csv' DELIMITER E'\t' CSV;
SELECT 'Copying data into Production.Culture';
\ copy Production.Culture
FROM 'Culture.csv' DELIMITER E'\t' CSV;
SELECT 'Copying data into Production.Document';
\ copy Production.Document
FROM 'Document.csv' DELIMITER E'\t' CSV;
SELECT 'Copying data into Production.ProductCategory';
\ copy Production.ProductCategory
FROM 'ProductCategory.csv' DELIMITER E'\t' CSV;
SELECT 'Copying data into Production.ProductSubcategory';
\ copy Production.ProductSubcategory
FROM 'ProductSubcategory.csv' DELIMITER E'\t' CSV;
SELECT 'Copying data into Production.ProductModel';
\ copy Production.ProductModel
FROM 'ProductModel.csv' DELIMITER E'\t' CSV;
SELECT 'Copying data into Production.Product';
\ copy Production.Product
FROM 'Product.csv' DELIMITER E'\t' CSV;
SELECT 'Copying data into Production.ProductCostHistory';
\ copy Production.ProductCostHistory
FROM 'ProductCostHistory.csv' DELIMITER E'\t' CSV;
SELECT 'Copying data into Production.ProductDescription';
\ copy Production.ProductDescription
FROM 'ProductDescription.csv' DELIMITER E'\t' CSV;
SELECT 'Copying data into Production.ProductDocument';
\ copy Production.ProductDocument
FROM 'ProductDocument.csv' DELIMITER E'\t' CSV;
SELECT 'Copying data into Production.Location';
\ copy Production.Location
FROM 'Location.csv' DELIMITER E'\t' CSV;
SELECT 'Copying data into Production.ProductInventory';
\ copy Production.ProductInventory
FROM 'ProductInventory.csv' DELIMITER E'\t' CSV;
SELECT 'Copying data into Production.ProductListPriceHistory';
\ copy Production.ProductListPriceHistory
FROM 'ProductListPriceHistory.csv' DELIMITER E'\t' CSV;
SELECT 'Copying data into Production.Illustration';
\ copy Production.Illustration
FROM 'Illustration.csv' DELIMITER E'\t' CSV;
SELECT 'Copying data into Production.ProductModelIllustration';
\ copy Production.ProductModelIllustration
FROM 'ProductModelIllustration.csv' DELIMITER E'\t' CSV;
SELECT 'Copying data into Production.ProductModelProductDescriptionCulture';
\ copy Production.ProductModelProductDescriptionCulture
FROM 'ProductModelProductDescriptionCulture.csv' DELIMITER E'\t' CSV;
SELECT 'Copying data into Production.ProductPhoto';
\ copy Production.ProductPhoto
FROM 'ProductPhoto.csv' DELIMITER E'\t' CSV;
SELECT 'Copying data into Production.ProductProductPhoto';
\ copy Production.ProductProductPhoto
FROM 'ProductProductPhoto.csv' DELIMITER E'\t' CSV;
-- This doesn't work:
...
\ copy Production.ScrapReason
FROM 'ScrapReason.csv' DELIMITER E'\t' CSV;
SELECT 'Copying data into Production.TransactionHistory';
\ copy Production.TransactionHistory
FROM 'TransactionHistory.csv' DELIMITER E'\t' CSV;
SELECT 'Copying data into Production.TransactionHistoryArchive';
\ copy Production.TransactionHistoryArchive
FROM 'TransactionHistoryArchive.csv' DELIMITER E'\t' CSV;
SELECT 'Copying data into Production.UnitMeasure';
\ copy Production.UnitMeasure
FROM 'UnitMeasure.csv' DELIMITER E'\t' CSV;
SELECT 'Copying data into Production.WorkOrder';
\ copy Production.WorkOrder
FROM 'WorkOrder.csv' DELIMITER E'\t' CSV;
SELECT 'Copying data into Production.WorkOrderRouting';
\ copy Production.WorkOrderRouting
FROM 'WorkOrderRouting.csv' DELIMITER E'\t' CSV;
...
SET DocumentNode = DocumentNode || SUBSTRING(doc, 3, 2)::bit(2)::INTEGER::VARCHAR || CASE
        SUBSTRING(doc, 5, 1)
        WHEN '0' THEN '.'
        ELSE '/'
    END,
...
-- 100 = 4-7
UPDATE Production.Document
SET DocumentNode = DocumentNode || (SUBSTRING(doc, 4, 2)::bit(2)::INTEGER + 4)::VARCHAR || CASE
        SUBSTRING(doc, 6, 1)
        WHEN '0' THEN '.'
        ELSE '/'
    END,
...
-- 101 = 8-15
UPDATE Production.Document
SET DocumentNode = DocumentNode || (SUBSTRING(doc, 4, 3)::bit(3)::INTEGER + 8)::VARCHAR || CASE
        SUBSTRING(doc, 7, 1)
        WHEN '0' THEN '.'
        ELSE '/'
    END,
...
SET DocumentNode = DocumentNode || (
        (
            SUBSTRING(doc, 4, 2) || SUBSTRING(doc, 7, 1) || SUBSTRING(doc, 9, 3)
        )::bit(6)::INTEGER + 16
    )::VARCHAR || CASE
        SUBSTRING(doc, 12, 1)
        WHEN '0' THEN '.'
...
SET DocumentNode = DocumentNode || (
        (
            SUBSTRING(doc, 5, 3) || SUBSTRING(doc, 9, 3) || SUBSTRING(doc, 13, 1) || SUBSTRING(doc, 15, 3)
        )::bit(10)::INTEGER + 80
    )::VARCHAR || CASE
        SUBSTRING(doc, 18, 1)
        WHEN '0' THEN '.'
...
CREATE SCHEMA Purchasing CREATE TABLE ProductVendor(
    ...
    -- money
    LineTotal numeric,
    -- AS ISNULL(OrderQty * UnitPrice, 0.00),
    ReceivedQty decimal(8, 2) NOT NULL,
    RejectedQty decimal(8, 2) NOT NULL,
    StockedQty numeric,
    -- AS ISNULL(ReceivedQty - RejectedQty, 0.00),
    ...
    -- int
    RevisionNumber smallint NOT NULL CONSTRAINT "DF_PurchaseOrderHeader_RevisionNumber" DEFAULT (0),
    -- tinyint
    Status smallint NOT NULL CONSTRAINT "DF_PurchaseOrderHeader_Status" DEFAULT (1),
    -- tinyint
    ...
    SubTotal numeric NOT NULL CONSTRAINT "DF_PurchaseOrderHeader_SubTotal" DEFAULT (0.00),
    -- money
    TaxAmt numeric NOT NULL CONSTRAINT "DF_PurchaseOrderHeader_TaxAmt" DEFAULT (0.00),
    -- money
    Freight numeric NOT NULL CONSTRAINT "DF_PurchaseOrderHeader_Freight" DEFAULT (0.00),
    -- money
    TotalDue numeric,
    -- AS ISNULL(SubTotal + TaxAmt + Freight, 0) PERSISTED NOT NULL,
    ModifiedDate TIMESTAMP NOT NULL CONSTRAINT "DF_PurchaseOrderHeader_ModifiedDate" DEFAULT (NOW()),
    CONSTRAINT "CK_PurchaseOrderHeader_Status" CHECK (
        Status BETWEEN 1 AND 4
    ),
    -- 1 = Pending; 2 = Approved; 3 = Rejected; 4 = Complete
    CONSTRAINT "CK_PurchaseOrderHeader_ShipDate" CHECK (
        (ShipDate >= OrderDate)
        OR (ShipDate IS NULL)
    ),
    ...
    -- int
    Name "Name" NOT NULL,
    ShipBase numeric NOT NULL CONSTRAINT "DF_ShipMethod_ShipBase" DEFAULT (0.00),
    -- money
    ShipRate numeric NOT NULL CONSTRAINT "DF_ShipMethod_ShipRate" DEFAULT (0.00),
    -- money
    rowguid uuid NOT NULL CONSTRAINT "DF_ShipMethod_rowguid" DEFAULT (uuid_generate_v1()),
    -- ROWGUIDCOL
    ...
    CONSTRAINT "CK_Vendor_CreditRating" CHECK (
...
\ copy Purchasing.ProductVendor
FROM 'ProductVendor.csv' DELIMITER E'\t' CSV;
SELECT 'Copying data into Purchasing.PurchaseOrderDetail';
\ copy Purchasing.PurchaseOrderDetail
FROM 'PurchaseOrderDetail.csv' DELIMITER E'\t' CSV;
SELECT 'Copying data into Purchasing.PurchaseOrderHeader';
\ copy Purchasing.PurchaseOrderHeader
FROM 'PurchaseOrderHeader.csv' DELIMITER E'\t' CSV;
SELECT 'Copying data into Purchasing.ShipMethod';
\ copy Purchasing.ShipMethod
FROM 'ShipMethod.csv' DELIMITER E'\t' CSV;
SELECT 'Copying data into Purchasing.Vendor';
\ copy Purchasing.Vendor
FROM 'Vendor.csv' DELIMITER E'\t' CSV;
-- Calculated columns that needed to be there just for the CSV import
...
CREATE SCHEMA Sales CREATE TABLE CountryRegionCurrency(
    CountryRegionCode varchar(3) NOT NULL,
    ...
    -- AS ISNULL('AW' + dbo.ufnLeadingZeros(CustomerID), ''),
    ...
    UnitPrice numeric NOT NULL,
    -- money
    UnitPriceDiscount numeric NOT NULL CONSTRAINT "DF_SalesOrderDetail_UnitPriceDiscount" DEFAULT (0.0),
    -- money
    LineTotal numeric,
    -- AS ISNULL(UnitPrice * (1.0 - UnitPriceDiscount) * OrderQty, 0.0),
    rowguid uuid NOT NULL CONSTRAINT "DF_SalesOrderDetail_rowguid" DEFAULT (uuid_generate_v1()),
    -- ROWGUIDCOL
    ...
    --  NOT FOR REPLICATION -- int
    RevisionNumber smallint NOT NULL CONSTRAINT "DF_SalesOrderHeader_RevisionNumber" DEFAULT (0),
    -- tinyint
    OrderDate TIMESTAMP NOT NULL CONSTRAINT "DF_SalesOrderHeader_OrderDate" DEFAULT (NOW()),
    DueDate TIMESTAMP NOT NULL,
    ShipDate TIMESTAMP NULL,
    Status smallint NOT NULL CONSTRAINT "DF_SalesOrderHeader_Status" DEFAULT (1),
    -- tinyint
    OnlineOrderFlag "Flag" NOT NULL CONSTRAINT "DF_SalesOrderHeader_OnlineOrderFlag" DEFAULT (true),
    SalesOrderNumber VARCHAR(23),
    -- AS ISNULL(N'SO' + CONVERT(nvarchar(23), SalesOrderID), N'*** ERROR ***'),
    ...
    SubTotal numeric NOT NULL CONSTRAINT "DF_SalesOrderHeader_SubTotal" DEFAULT (0.00),
    -- money
    TaxAmt numeric NOT NULL CONSTRAINT "DF_SalesOrderHeader_TaxAmt" DEFAULT (0.00),
    -- money
    Freight numeric NOT NULL CONSTRAINT "DF_SalesOrderHeader_Freight" DEFAULT (0.00),
    -- money
    TotalDue numeric,
    -- AS ISNULL(SubTotal + TaxAmt + Freight, 0),
    Comment varchar(128) NULL,
    rowguid uuid NOT NULL CONSTRAINT "DF_SalesOrderHeader_rowguid" DEFAULT (uuid_generate_v1()),
    -- ROWGUIDCOL
    ModifiedDate TIMESTAMP NOT NULL CONSTRAINT "DF_SalesOrderHeader_ModifiedDate" DEFAULT (NOW()),
    CONSTRAINT "CK_SalesOrderHeader_Status" CHECK (
        Status BETWEEN 0 AND 8
    ),
    CONSTRAINT "CK_SalesOrderHeader_DueDate" CHECK (DueDate >= OrderDate),
    CONSTRAINT "CK_SalesOrderHeader_ShipDate" CHECK (
        (ShipDate >= OrderDate)
        OR (ShipDate IS NULL)
    ),
    ...
    BusinessEntityID INT NOT NULL,
    TerritoryID INT NULL,
    SalesQuota numeric NULL,
    -- money
    Bonus numeric NOT NULL CONSTRAINT "DF_SalesPerson_Bonus" DEFAULT (0.00),
    -- money
    CommissionPct numeric NOT NULL CONSTRAINT "DF_SalesPerson_CommissionPct" DEFAULT (0.00),
    -- smallmoney -- money
    SalesYTD numeric NOT NULL CONSTRAINT "DF_SalesPerson_SalesYTD" DEFAULT (0.00),
    -- money
    SalesLastYear numeric NOT NULL CONSTRAINT "DF_SalesPerson_SalesLastYear" DEFAULT (0.00),
    -- money
    rowguid uuid NOT NULL CONSTRAINT "DF_SalesPerson_rowguid" DEFAULT (uuid_generate_v1()),
    -- ROWGUIDCOL
    ...
    -- money
    rowguid uuid NOT NULL CONSTRAINT "DF_SalesPersonQuotaHistory_rowguid" DEFAULT (uuid_generate_v1()),
    -- ROWGUIDCOL
    ...
    -- tinyint
    TaxRate numeric NOT NULL CONSTRAINT "DF_SalesTaxRate_TaxRate" DEFAULT (0.00),
    -- smallmoney -- money
    Name "Name" NOT NULL,
    rowguid uuid NOT NULL CONSTRAINT "DF_SalesTaxRate_rowguid" DEFAULT (uuid_generate_v1()),
    -- ROWGUIDCOL
    ModifiedDate TIMESTAMP NOT NULL CONSTRAINT "DF_SalesTaxRate_ModifiedDate" DEFAULT (NOW()),
    CONSTRAINT "CK_SalesTaxRate_TaxType" CHECK (
    ...
    -- Group
    SalesYTD numeric NOT NULL CONSTRAINT "DF_SalesTerritory_SalesYTD" DEFAULT (0.00),
    -- money
    SalesLastYear numeric NOT NULL CONSTRAINT "DF_SalesTerritory_SalesLastYear" DEFAULT (0.00),
    -- money
    CostYTD numeric NOT NULL CONSTRAINT "DF_SalesTerritory_CostYTD" DEFAULT (0.00),
    -- money
    CostLastYear numeric NOT NULL CONSTRAINT "DF_SalesTerritory_CostLastYear" DEFAULT (0.00),
    -- money
    rowguid uuid NOT NULL CONSTRAINT "DF_SalesTerritory_rowguid" DEFAULT (uuid_generate_v1()),
    -- ROWGUIDCOL
    ...
    StartDate TIMESTAMP NOT NULL,
    EndDate TIMESTAMP NULL,
    rowguid uuid NOT NULL CONSTRAINT "DF_SalesTerritoryHistory_rowguid" DEFAULT (uuid_generate_v1()),
    -- ROWGUIDCOL
    ModifiedDate TIMESTAMP NOT NULL CONSTRAINT "DF_SalesTerritoryHistory_ModifiedDate" DEFAULT (NOW()),
    CONSTRAINT "CK_SalesTerritoryHistory_EndDate" CHECK (
        (EndDate >= StartDate)
        OR (EndDate IS NULL)
    )
    ...
    DiscountPct numeric NOT NULL CONSTRAINT "DF_SpecialOffer_DiscountPct" DEFAULT (0.00),
    -- smallmoney -- money
    ...
    MaxQty INT NULL,
    rowguid uuid NOT NULL CONSTRAINT "DF_SpecialOffer_rowguid" DEFAULT (uuid_generate_v1()),
    -- ROWGUIDCOL
    ...
    SpecialOfferID INT NOT NULL,
    ProductID INT NOT NULL,
    rowguid uuid NOT NULL CONSTRAINT "DF_SpecialOfferProduct_rowguid" DEFAULT (uuid_generate_v1()),
    -- ROWGUIDCOL
...
\ copy Sales.CountryRegionCurrency
FROM 'CountryRegionCurrency.csv' DELIMITER E'\t' CSV;
SELECT 'Copying data into Sales.CreditCard';
\ copy Sales.CreditCard
FROM 'CreditCard.csv' DELIMITER E'\t' CSV;
SELECT 'Copying data into Sales.Currency';
\ copy Sales.Currency
FROM 'Currency.csv' DELIMITER E'\t' CSV;
SELECT 'Copying data into Sales.CurrencyRate';
\ copy Sales.CurrencyRate
FROM 'CurrencyRate.csv' DELIMITER E'\t' CSV;
SELECT 'Copying data into Sales.Customer';
\ copy Sales.Customer
FROM 'Customer.csv' DELIMITER E'\t' CSV;
SELECT 'Copying data into Sales.PersonCreditCard';
\ copy Sales.PersonCreditCard
FROM 'PersonCreditCard.csv' DELIMITER E'\t' CSV;
SELECT 'Copying data into Sales.SalesOrderDetail';
\ copy Sales.SalesOrderDetail
FROM 'SalesOrderDetail.csv' DELIMITER E'\t' CSV;
SELECT 'Copying data into Sales.SalesOrderHeader';
\ copy Sales.SalesOrderHeader
FROM 'SalesOrderHeader.csv' DELIMITER E'\t' CSV;
SELECT 'Copying data into Sales.SalesOrderHeaderSalesReason';
\ copy Sales.SalesOrderHeaderSalesReason
FROM 'SalesOrderHeaderSalesReason.csv' DELIMITER E'\t' CSV;
SELECT 'Copying data into Sales.SalesPerson';
\ copy Sales.SalesPerson
FROM 'SalesPerson.csv' DELIMITER E'\t' CSV;
SELECT 'Copying data into Sales.SalesPersonQuotaHistory';
\ copy Sales.SalesPersonQuotaHistory
FROM 'SalesPersonQuotaHistory.csv' DELIMITER E'\t' CSV;
SELECT 'Copying data into Sales.SalesReason';
\ copy Sales.SalesReason
FROM 'SalesReason.csv' DELIMITER E'\t' CSV;
SELECT 'Copying data into Sales.SalesTaxRate';
\ copy Sales.SalesTaxRate
FROM 'SalesTaxRate.csv' DELIMITER E'\t' CSV;
SELECT 'Copying data into Sales.SalesTerritory';
\ copy Sales.SalesTerritory
FROM 'SalesTerritory.csv' DELIMITER E'\t' CSV;
SELECT 'Copying data into Sales.SalesTerritoryHistory';
\ copy Sales.SalesTerritoryHistory
FROM 'SalesTerritoryHistory.csv' DELIMITER E'\t' CSV;
SELECT 'Copying data into Sales.ShoppingCartItem';
\ copy Sales.ShoppingCartItem
FROM 'ShoppingCartItem.csv' DELIMITER E'\t' CSV;
SELECT 'Copying data into Sales.SpecialOffer';
\ copy Sales.SpecialOffer
FROM 'SpecialOffer.csv' DELIMITER E'\t' CSV;
SELECT 'Copying data into Sales.SpecialOfferProduct';
\ copy Sales.SpecialOfferProduct
FROM 'SpecialOfferProduct.csv' DELIMITER E'\t' CSV;
SELECT 'Copying data into Sales.Store';
\ copy Sales.Store
FROM 'Store.csv' DELIMITER E'\t' CSV;
-- Calculated columns that needed to be there just for the CSV import
...
ADD CONSTRAINT "PK_EmployeeDepartmentHistory_BusinessEntityID_StartDate_Departm" PRIMARY KEY (
        BusinessEntityID,
        StartDate,
        DepartmentID,
        ShiftID
...
ADD CONSTRAINT "PK_SalesTerritoryHistory_BusinessEntityID_StartDate_TerritoryID" PRIMARY KEY (
        BusinessEntityID,
        --Sales person,
        StartDate,
        TerritoryID
...
(
        xpath(
            '(act:telephoneNumber)[1]/act:number/text()',
            node,
    ...
    BTRIM(
        (
            xpath(
                '(act:telephoneNumber)[1]/act:SpecialInstructions/text()',
                node,
                '{{act,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/ContactTypes}}'
            )
        ) [1]::VARCHAR
...
(
        xpath(
            '(act:homePostalAddress)[1]/act:Street/text()',
            node,
            '{{act,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/ContactTypes}}'
        )
    ) [1] AS Street,
(
        xpath(
            '(act:homePostalAddress)[1]/act:City/text()',
            node,
            '{{act,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/ContactTypes}}'
        )
    ) [1] AS City,
(
        xpath(
            '(act:homePostalAddress)[1]/act:StateProvince/text()',
            node,
            '{{act,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/ContactTypes}}'
        )
    ) [1] AS StateProvince,
(
        xpath(
            '(act:homePostalAddress)[1]/act:PostalCode/text()',
            node,
            '{{act,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/ContactTypes}}'
        )
    ) [1] AS PostalCode,
(
        xpath(
            '(act:homePostalAddress)[1]/act:CountryRegion/text()',
            node,
            '{{act,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/ContactTypes}}'
        )
    ) [1] AS CountryRegion,
(
        xpath(
            '(act:homePostalAddress)[1]/act:SpecialInstructions/text()',
            node,
            '{{act,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/ContactTypes}}'
        )
    ) [1] AS HomeAddressSpecialInstructions,
(
        xpath(
            '(act:eMail)[1]/act:eMailAddress/text()',
            node,
            '{{act,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/ContactTypes}}'
        )
    ) [1] AS EMailAddress,
    BTRIM(
        (
            xpath(
                '(act:eMail)[1]/act:SpecialInstructions/text()',
                node,
                '{{act,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/ContactTypes}}'
            )
        ) [1]::VARCHAR
...
(
        xpath(
            '((act:eMail)[1]/act:SpecialInstructions/act:telephoneNumber)[1]/act:number/text()',
            node,
            '{{act,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/ContactTypes}}'
        )
    ) [1] AS EMailTelephoneNumber,
    p.rowguid,
    p.ModifiedDate
FROM Person.Person AS p
    LEFT OUTER JOIN (
        SELECT BusinessEntityID,
            UNNEST(
                xpath(
                    ...
                    '{{ci,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/ContactInfo}}'
                )
            ) AS node
        FROM Person.Person
        WHERE AdditionalContactInfo IS NOT NULL
    ) AS additional ON p.BusinessEntityID = additional.BusinessEntityID;
...
SELECT BusinessEntityID,
    CAST(
        (
            xpath(
                'n:TotalPurchaseYTD/text()',
                Demographics,
                '{{n,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/IndividualSurvey}}'
            )
        ) [1]::VARCHAR AS money
    ) AS TotalPurchaseYTD,
    CAST(
...
(
        xpath(
            'n:MaritalStatus/text()',
            Demographics,
            '{{n,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/IndividualSurvey}}'
        )
    ) [1]::VARCHAR(1) AS MaritalStatus,
(
        xpath(
            'n:YearlyIncome/text()',
            Demographics,
...
(
    ...
    ) [1]::VARCHAR(1) AS Gender,
    CAST(
        (
            xpath(
                'n:TotalChildren/text()',
                Demographics,
                '{{n,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/IndividualSurvey}}'
            )
        ) [1]::VARCHAR AS INTEGER
    ) AS TotalChildren,
    CAST(
        (
            xpath(
                'n:NumberChildrenAtHome/text()',
                Demographics,
                '{{n,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/IndividualSurvey}}'
            )
        ) [1]::VARCHAR AS INTEGER
...
(
    ...
    ) [1]::VARCHAR(30) AS Occupation,
    CAST(
        (
            xpath(
                'n:HomeOwnerFlag/text()',
                Demographics,
                '{{n,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/IndividualSurvey}}'
            )
        ) [1]::VARCHAR AS BOOLEAN
    ) AS HomeOwnerFlag,
    CAST(
        (
            xpath(
                'n:NumberCarsOwned/text()',
                Demographics,
                '{{n,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/IndividualSurvey}}'
            )
        ) [1]::VARCHAR AS INTEGER
...
(
        xpath(
            '/n:Resume/n:Name/n:Name.Prefix/text()',
            Resume,
            '{{n,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/Resume}}'
        )
    ) [1]::varchar(30) AS "Name.Prefix",
(
        xpath(
            '/n:Resume/n:Name/n:Name.First/text()',
            Resume,
            '{{n,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/Resume}}'
        )
    ) [1]::varchar(30) AS "Name.First",
(
        xpath(
            '/n:Resume/n:Name/n:Name.Middle/text()',
            Resume,
            '{{n,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/Resume}}'
        )
    ) [1]::varchar(30) AS "Name.Middle",
(
        xpath(
            '/n:Resume/n:Name/n:Name.Last/text()',
            Resume,
            '{{n,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/Resume}}'
        )
    ) [1]::varchar(30) AS "Name.Last",
(
        xpath(
            '/n:Resume/n:Name/n:Name.Suffix/text()',
            Resume,
            '{{n,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/Resume}}'
        )
    ) [1]::varchar(30) AS "Name.Suffix",
(
        xpath(
            '/n:Resume/n:Skills/text()',
            Resume,
            '{{n,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/Resume}}'
        )
    ) [1]::varchar AS "Skills",
(
        xpath(
            'n:Address/n:Addr.Type/text()',
            Resume,
            '{{n,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/Resume}}'
        )
    ) [1]::varchar(30) AS "Addr.Type",
(
        xpath(
            'n:Address/n:Addr.Location/n:Location/n:Loc.CountryRegion/text()',
            Resume,
            '{{n,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/Resume}}'
        )
    ) [1]::varchar(100) AS "Addr.Loc.CountryRegion",
(
        xpath(
            'n:Address/n:Addr.Location/n:Location/n:Loc.State/text()',
            Resume,
            '{{n,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/Resume}}'
        )
    ) [1]::varchar(100) AS "Addr.Loc.State",
(
        xpath(
            'n:Address/n:Addr.Location/n:Location/n:Loc.City/text()',
            Resume,
            '{{n,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/Resume}}'
        )
    ) [1]::varchar(100) AS "Addr.Loc.City",
(
        xpath(
            'n:Address/n:Addr.PostalCode/text()',
            Resume,
            '{{n,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/Resume}}'
        )
    ) [1]::varchar(20) AS "Addr.PostalCode",
(
        xpath(
            '/n:Resume/n:EMail/text()',
            Resume,
            '{{n,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/Resume}}'
        )
    ) [1]::varchar AS "EMail",
(
        xpath(
            '/n:Resume/n:WebSite/text()',
            Resume,
            '{{n,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/Resume}}'
        )
    ) [1]::varchar AS "WebSite",
    ModifiedDate
...
SELECT JobCandidateID,
    CAST(
        UNNEST(
            xpath(
                '/ns:Resume/ns:Employment/ns:Emp.StartDate/text()',
                Resume,
    ...
    CAST(
        UNNEST(
            xpath(
                '/ns:Resume/ns:Employment/ns:Emp.EndDate/text()',
                Resume,
                '{{ns,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/Resume}}'
            )
        )::VARCHAR(20) AS DATE
    ...
    UNNEST(
        xpath(
            '/ns:Resume/ns:Employment/ns:Emp.OrgName/text()',
            Resume,
            '{{ns,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/Resume}}'
        )
    )::varchar(100) AS "Emp.OrgName",
    UNNEST(
        xpath(
            '/ns:Resume/ns:Employment/ns:Emp.JobTitle/text()',
            Resume,
            '{{ns,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/Resume}}'
        )
    )::varchar(100) AS "Emp.JobTitle",
    UNNEST(
        xpath(
            '/ns:Resume/ns:Employment/ns:Emp.Responsibility/text()',
            Resume,
            '{{ns,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/Resume}}'
        )
    )::varchar AS "Emp.Responsibility",
    UNNEST(
        xpath(
            '/ns:Resume/ns:Employment/ns:Emp.FunctionCategory/text()',
            Resume,
            '{{ns,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/Resume}}'
        )
    )::varchar AS "Emp.FunctionCategory",
    UNNEST(
        xpath(
            '/ns:Resume/ns:Employment/ns:Emp.IndustryCategory/text()',
            Resume,
            '{{ns,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/Resume}}'
        )
    )::varchar AS "Emp.IndustryCategory",
    UNNEST(
        xpath(
            '/ns:Resume/ns:Employment/ns:Emp.Location/ns:Location/ns:Loc.CountryRegion/text()',
            Resume,
            '{{ns,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/Resume}}'
        )
    )::varchar AS "Emp.Loc.CountryRegion",
    UNNEST(
        xpath(
            '/ns:Resume/ns:Employment/ns:Emp.Location/ns:Location/ns:Loc.State/text()',
            Resume,
            '{{ns,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/Resume}}'
        )
    )::varchar AS "Emp.Loc.State",
    UNNEST(
        xpath(
            '/ns:Resume/ns:Employment/ns:Emp.Location/ns:Location/ns:Loc.City/text()',
            Resume,
            '{{ns,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/Resume}}'
        )
    )::varchar AS "Emp.Loc.City"
FROM HumanResources.JobCandidate;
...
(
        xpath(
            '/root/ns:Education/ns:Edu.Level/text()',
            jc.doc,
            '{{ns,http://adventureworks.com}}'
        )
    ) [1]::varchar(50) AS "Edu.Level",
    CAST(
        (
            xpath(
                '/root/ns:Education/ns:Edu.StartDate/text()',
                jc.doc,
                '{{ns,http://adventureworks.com}}'
            )
        ) [1]::VARCHAR(20) AS DATE
    ) AS "Edu.StartDate",
    CAST(
        (
            xpath(
                '/root/ns:Education/ns:Edu.EndDate/text()',
                jc.doc,
                '{{ns,http://adventureworks.com}}'
            )
        ) [1]::VARCHAR(20) AS DATE
...
(
        xpath(
            '/root/ns:Education/ns:Edu.Degree/text()',
            jc.doc,
            '{{ns,http://adventureworks.com}}'
        )
    ) [1]::varchar(50) AS "Edu.Degree",
(
        xpath(
            '/root/ns:Education/ns:Edu.Major/text()',
            jc.doc,
            '{{ns,http://adventureworks.com}}'
        )
    ) [1]::varchar(50) AS "Edu.Major",
(
        xpath(
            '/root/ns:Education/ns:Edu.Minor/text()',
            jc.doc,
            '{{ns,http://adventureworks.com}}'
        )
    ) [1]::varchar(50) AS "Edu.Minor",
(
        xpath(
            '/root/ns:Education/ns:Edu.GPA/text()',
            jc.doc,
            '{{ns,http://adventureworks.com}}'
        )
    ) [1]::varchar(5) AS "Edu.GPA",
(
        xpath(
            '/root/ns:Education/ns:Edu.GPAScale/text()',
            jc.doc,
            '{{ns,http://adventureworks.com}}'
        )
    ) [1]::varchar(5) AS "Edu.GPAScale",
(
        xpath(
            '/root/ns:Education/ns:Edu.School/text()',
            jc.doc,
            '{{ns,http://adventureworks.com}}'
        )
    ) [1]::varchar(100) AS "Edu.School",
(
        xpath(
            '/root/ns:Education/ns:Edu.Location/ns:Location/ns:Loc.CountryRegion/text()',
            jc.doc,
            '{{ns,http://adventureworks.com}}'
        )
    ) [1]::varchar(100) AS "Edu.Loc.CountryRegion",
(
        xpath(
            '/root/ns:Education/ns:Edu.Location/ns:Location/ns:Loc.State/text()',
            jc.doc,
            '{{ns,http://adventureworks.com}}'
        )
    ) [1]::varchar(100) AS "Edu.Loc.State",
(
        xpath(
            '/root/ns:Education/ns:Edu.Location/ns:Location/ns:Loc.City/text()',
            jc.doc,
            '{{ns,http://adventureworks.com}}'
        )
    ) [1]::varchar(100) AS "Edu.Loc.City"
FROM (
        SELECT JobCandidateID -- Because the underlying XML data used in this example has namespaces defined at the document level,
...
,
(
                '<root xmlns:ns="http://adventureworks.com">' || unnesting.Education::varchar || '</root>'
            )::xml AS doc
        FROM (
                SELECT JobCandidateID,
                    UNNEST(
...
(
        xpath(
            '/p1:ProductDescription/p1:Summary/html:p/text()',
            CatalogDescription,
            '{{p1,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/ProductModelDescription},{html,http://www.w3.org/1999/xhtml}}'
        )
    ) [1]::varchar AS "Summary",
(
        xpath(
            '/p1:ProductDescription/p1:Manufacturer/p1:Name/text()',
            CatalogDescription,
            '{{p1,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/ProductModelDescription}}'
        )
    ) [1]::varchar AS Manufacturer,
(
        xpath(
            '/p1:ProductDescription/p1:Manufacturer/p1:Copyright/text()',
            CatalogDescription,
            '{{p1,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/ProductModelDescription}}'
        )
    ) [1]::varchar(30) AS Copyright,
(
        xpath(
            '/p1:ProductDescription/p1:Manufacturer/p1:ProductURL/text()',
            CatalogDescription,
            '{{p1,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/ProductModelDescription}}'
        )
    ) [1]::varchar(256) AS ProductURL,
(
        xpath(
            '/p1:ProductDescription/p1:Features/wm:Warranty/wm:WarrantyPeriod/text()',
            CatalogDescription,
            '{{p1,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/ProductModelDescription},{wm,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/ProductModelWarrAndMain}}'
        )
    ) [1]::varchar(256) AS WarrantyPeriod,
(
        xpath(
            '/p1:ProductDescription/p1:Features/wm:Warranty/wm:Description/text()',
            CatalogDescription,
            '{{p1,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/ProductModelDescription},{wm,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/ProductModelWarrAndMain}}'
        )
...
(
        xpath(
            '/p1:ProductDescription/p1:Features/wm:Maintenance/wm:NoOfYears/text()',
            CatalogDescription,
            '{{p1,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/ProductModelDescription},{wm,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/ProductModelWarrAndMain}}'
        )
    ) [1]::varchar(256) AS NoOfYears,
(
        xpath(
            '/p1:ProductDescription/p1:Features/wm:Maintenance/wm:Description/text()',
            CatalogDescription,
            '{{p1,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/ProductModelDescription},{wm,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/ProductModelWarrAndMain}}'
        )
...
(
        xpath(
            '/p1:ProductDescription/p1:Features/wf:wheel/text()',
            CatalogDescription,
            '{{p1,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/ProductModelDescription},{wf,http://www.adventure-works.com/schemas/OtherFeatures}}'
        )
    ) [1]::varchar(256) AS Wheel,
(
        xpath(
            '/p1:ProductDescription/p1:Features/wf:saddle/text()',
            CatalogDescription,
            '{{p1,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/ProductModelDescription},{wf,http://www.adventure-works.com/schemas/OtherFeatures}}'
        )
    ) [1]::varchar(256) AS Saddle,
(
        xpath(
            '/p1:ProductDescription/p1:Features/wf:pedal/text()',
            CatalogDescription,
            '{{p1,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/ProductModelDescription},{wf,http://www.adventure-works.com/schemas/OtherFeatures}}'
        )
    ) [1]::varchar(256) AS Pedal,
(
        xpath(
            '/p1:ProductDescription/p1:Features/wf:BikeFrame/text()',
            CatalogDescription,
            '{{p1,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/ProductModelDescription},{wf,http://www.adventure-works.com/schemas/OtherFeatures}}'
        )
    ) [1]::varchar AS BikeFrame,
(
        xpath(
            '/p1:ProductDescription/p1:Features/wf:crankset/text()',
            CatalogDescription,
            '{{p1,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/ProductModelDescription},{wf,http://www.adventure-works.com/schemas/OtherFeatures}}'
        )
    ) [1]::varchar(256) AS Crankset,
(
        xpath(
            '/p1:ProductDescription/p1:Picture/p1:Angle/text()',
            CatalogDescription,
            '{{p1,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/ProductModelDescription}}'
        )
    ) [1]::varchar(256) AS PictureAngle,
(
        xpath(
            '/p1:ProductDescription/p1:Picture/p1:Size/text()',
            CatalogDescription,
            '{{p1,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/ProductModelDescription}}'
        )
    ) [1]::varchar(256) AS PictureSize,
(
        xpath(
            '/p1:ProductDescription/p1:Picture/p1:ProductPhotoID/text()',
            CatalogDescription,
            '{{p1,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/ProductModelDescription}}'
        )
    ) [1]::varchar(256) AS ProductPhotoID,
(
        xpath(
            '/p1:ProductDescription/p1:Specifications/Material/text()',
            CatalogDescription,
            '{{p1,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/ProductModelDescription}}'
        )
    ) [1]::varchar(256) AS Material,
(
        xpath(
            '/p1:ProductDescription/p1:Specifications/Color/text()',
            CatalogDescription,
            '{{p1,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/ProductModelDescription}}'
        )
    ) [1]::varchar(256) AS Color,
(
        xpath(
            '/p1:ProductDescription/p1:Specifications/ProductLine/text()',
            CatalogDescription,
            '{{p1,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/ProductModelDescription}}'
        )
    ) [1]::varchar(256) AS ProductLine,
(
        xpath(
            '/p1:ProductDescription/p1:Specifications/Style/text()',
            CatalogDescription,
            '{{p1,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/ProductModelDescription}}'
        )
    ) [1]::varchar(256) AS Style,
(
        xpath(
            '/p1:ProductDescription/p1:Specifications/RiderExperience/text()',
            CatalogDescription,
            '{{p1,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/ProductModelDescription}}'
        )
    ) [1]::varchar(1024) AS RiderExperience,
    rowguid,
    ModifiedDate
...
SELECT pm.ProductModelID,
    pm.Name -- Access the overall Instructions xml brought through from %line 2938 and %line 2943
,
(
        xpath(
            '/ns:root/text()',
    ...
    ) [1]::varchar AS Instructions -- Bring out information about the location, broken out in %line 2945
,
    CAST(
        (xpath('@LocationID', pm.MfgInstructions)) [1]::varchar AS INTEGER
    ) AS "LocationID",
    CAST(
        (xpath('@SetupHours', pm.MfgInstructions)) [1]::varchar AS DECIMAL(9, 4)
    ) AS "SetupHours",
    CAST(
        (xpath('@MachineHours', pm.MfgInstructions)) [1]::varchar AS DECIMAL(9, 4)
    ) AS "MachineHours",
    CAST(
        (xpath('@LaborHours', pm.MfgInstructions)) [1]::varchar AS DECIMAL(9, 4)
    ) AS "LaborHours",
    CAST(
        (xpath('@LotSize', pm.MfgInstructions)) [1]::varchar AS INTEGER
    ) AS "LotSize" -- Show specific detail about each step broken out in %line 2940
,
(xpath('/step/text()', pm.Step)) [1]::varchar(1024) AS "Step",
    pm.rowguid,
    pm.ModifiedDate
FROM (
        SELECT locations.ProductModelID,
            locations.Name,
            locations.rowguid,
            locations.ModifiedDate,
            locations.Instructions,
            locations.MfgInstructions -- Further break out the location information from the inner query below into individual steps
,
            UNNEST(xpath('step', locations.MfgInstructions)) AS Step
        FROM (
                SELECT -- Just pass these through so they can be referenced at the outermost query
                    ProductModelID,
                    Name,
                    rowguid,
                    ModifiedDate,
                    Instructions -- And also break out Instructions into individual locations to pass up to the middle query
,
                    UNNEST(
                        xpath(
                            '/ns:root/ns:Location',
...
CREATE VIEW Sales.vSalesPersonSalesByFiscalYearsData AS -- Of the 56 possible combinations of one of the 14 SalesPersons selling across one of
...
SELECT granular.SalesPersonID,
    ...
    granular.FiscalYear
FROM -- Brings back 3703 rows of data -- there are 3806 total sales done by a SalesPerson,
    ...
    (
        SELECT soh.SalesPersonID,
            ...
            EXTRACT(
                YEAR
                FROM soh.OrderDate + '6 months'::interval
            ) AS FiscalYear
...
GROUP BY granular.SalesPersonID,
    granular.FullName,
    granular.JobTitle,
    granular.SalesTerritory,
    granular.FiscalYear;
...
ORDER BY 2,4' -- This set of fiscal years could have dynamically come from a SELECT DISTINCT,
...
SELECT BusinessEntityID,
    ...
    CAST(
        UNNEST(
            xpath(
                '/ns:StoreSurvey/ns:AnnualSales/text()',
                Demographics,
    ...
    CAST(
        UNNEST(
            xpath(
                '/ns:StoreSurvey/ns:AnnualRevenue/text()',
                Demographics,
                '{{ns,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/StoreSurvey}}'
            )
        )::varchar AS money
    ...
    UNNEST(
        xpath(
            '/ns:StoreSurvey/ns:BankName/text()',
            Demographics,
            '{{ns,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/StoreSurvey}}'
        )
    )::varchar(50) AS "BankName",
    UNNEST(
        xpath(
    ...
    UNNEST(
        xpath(
            '/ns:StoreSurvey/ns:Specialty/text()',
            Demographics,
            '{{ns,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/StoreSurvey}}'
        )
    )::varchar(50) AS "Specialty",
    CAST(
        UNNEST(
            xpath(
    ...
    UNNEST(
        xpath(
            '/ns:StoreSurvey/ns:Brands/text()',
            Demographics,
            '{{ns,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/StoreSurvey}}'
        )
    )::varchar(30) AS "Brands",
    UNNEST(
        xpath(
            '/ns:StoreSurvey/ns:Internet/text()',
            Demographics,
            '{{ns,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/StoreSurvey}}'
        )
    )::varchar(30) AS "Internet",
    CAST(
        UNNEST(
            xpath(
                '/ns:StoreSurvey/ns:NumberEmployees/text()',
                Demographics,
                '{{ns,http://schemas.microsoft.com/sqlserver/2004/07/adventure-works/StoreSurvey}}'
            )
        )::varchar AS integer
...
FROM Purchasing.Vendor v
    ...
    INNER JOIN Person.AddressType at ON at.AddressTypeID = bea.AddressTypeID;
-- Convenience views
CREATE SCHEMA pe CREATE VIEW a AS
SELECT addressid AS id,
    *
FROM person.address CREATE VIEW at AS
SELECT addresstypeid AS id,
    *
FROM person.addresstype CREATE VIEW be AS
SELECT businessentityid AS id,
    *
FROM person.businessentity CREATE VIEW bea AS
SELECT businessentityid AS id,
    *
FROM person.businessentityaddress CREATE VIEW bec AS
SELECT businessentityid AS id,
    *
FROM person.businessentitycontact CREATE VIEW ct AS
SELECT contacttypeid AS id,
    *
FROM person.contacttype CREATE VIEW cr AS
SELECT *
FROM person.countryregion CREATE VIEW e AS
SELECT emailaddressid AS id,
    *
FROM person.emailaddress CREATE VIEW pa AS
SELECT businessentityid AS id,
    *
FROM person.password CREATE VIEW p AS
SELECT businessentityid AS id,
    *
FROM person.person CREATE VIEW pp AS
SELECT businessentityid AS id,
    *
FROM person.personphone CREATE VIEW pnt AS
SELECT phonenumbertypeid AS id,
    *
FROM person.phonenumbertype CREATE VIEW sp AS
SELECT stateprovinceid AS id,
    *
FROM person.stateprovince;
CREATE SCHEMA hr CREATE VIEW d AS
SELECT departmentid AS id,
    *
FROM humanresources.department CREATE VIEW e AS
SELECT businessentityid AS id,
    *
FROM humanresources.employee CREATE VIEW edh AS
SELECT businessentityid AS id,
    *
FROM humanresources.employeedepartmenthistory CREATE VIEW eph AS
SELECT businessentityid AS id,
    *
FROM humanresources.employeepayhistory CREATE VIEW jc AS
SELECT jobcandidateid AS id,
    *
FROM humanresources.jobcandidate CREATE VIEW s AS
SELECT shiftid AS id,
    *
FROM humanresources.shift;
CREATE SCHEMA pr CREATE VIEW bom AS
SELECT billofmaterialsid AS id,
    *
FROM production.billofmaterials CREATE VIEW c AS
SELECT cultureid AS id,
    *
FROM production.culture CREATE VIEW d AS
SELECT *
FROM production.document CREATE VIEW i AS
SELECT illustrationid AS id,
    *
FROM production.illustration CREATE VIEW l AS
SELECT locationid AS id,
    *
FROM production.location CREATE VIEW p AS
SELECT productid AS id,
    *
FROM production.product CREATE VIEW pc AS
SELECT productcategoryid AS id,
    *
FROM production.productcategory CREATE VIEW pch AS
SELECT productid AS id,
    *
FROM production.productcosthistory CREATE VIEW pd AS
SELECT productdescriptionid AS id,
    *
FROM production.productdescription CREATE VIEW pdoc AS
SELECT productid AS id,
    *
FROM production.productdocument CREATE VIEW pi AS
SELECT productid AS id,
    *
FROM production.productinventory CREATE VIEW plph AS
SELECT productid AS id,
    *
FROM production.productlistpricehistory CREATE VIEW pm AS
SELECT productmodelid AS id,
    *
FROM production.productmodel CREATE VIEW pmi AS
SELECT *
FROM production.productmodelillustration CREATE VIEW pmpdc AS
SELECT *
FROM production.productmodelproductdescriptionculture CREATE VIEW pp AS
SELECT productphotoid AS id,
    *
FROM production.productphoto CREATE VIEW ppp AS
SELECT *
FROM production.productproductphoto CREATE VIEW pr AS
SELECT productreviewid AS id,
    *
FROM production.productreview CREATE VIEW psc AS
SELECT productsubcategoryid AS id,
    *
FROM production.productsubcategory CREATE VIEW sr AS
SELECT scrapreasonid AS id,
    *
FROM production.scrapreason CREATE VIEW th AS
SELECT transactionid AS id,
    *
FROM production.transactionhistory CREATE VIEW tha AS
SELECT transactionid AS id,
    *
FROM production.transactionhistoryarchive CREATE VIEW um AS
SELECT unitmeasurecode AS id,
    *
FROM production.unitmeasure CREATE VIEW w AS
SELECT workorderid AS id,
    *
FROM production.workorder CREATE VIEW wr AS
SELECT workorderid AS id,
    *
FROM production.workorderrouting;
CREATE SCHEMA pu CREATE VIEW pv AS
SELECT productid AS id,
    *
FROM purchasing.productvendor CREATE VIEW pod AS
SELECT purchaseorderdetailid AS id,
    *
FROM purchasing.purchaseorderdetail CREATE VIEW poh AS
SELECT purchaseorderid AS id,
    *
FROM purchasing.purchaseorderheader CREATE VIEW sm AS
SELECT shipmethodid AS id,
    *
FROM purchasing.shipmethod CREATE VIEW v AS
SELECT businessentityid AS id,
    *
FROM purchasing.vendor;
CREATE SCHEMA sa CREATE VIEW crc AS
SELECT *
FROM sales.countryregioncurrency CREATE VIEW cc AS
SELECT creditcardid AS id,
    *
FROM sales.creditcard CREATE VIEW cu AS
SELECT currencycode AS id,
    *
FROM sales.currency CREATE VIEW cr AS
SELECT *
FROM sales.currencyrate CREATE VIEW c AS
SELECT customerid AS id,
    *
FROM sales.customer CREATE VIEW pcc AS
SELECT businessentityid AS id,
    *
FROM sales.personcreditcard CREATE VIEW sod AS
SELECT salesorderdetailid AS id,
    *
FROM sales.salesorderdetail CREATE VIEW soh AS
SELECT salesorderid AS id,
    *
FROM sales.salesorderheader CREATE VIEW sohsr AS
SELECT *
FROM sales.salesorderheadersalesreason CREATE VIEW sp AS
SELECT businessentityid AS id,
    *
FROM sales.salesperson CREATE VIEW spqh AS
SELECT businessentityid AS id,
    *
FROM sales.salespersonquotahistory CREATE VIEW sr AS
SELECT salesreasonid AS id,
    *
FROM sales.salesreason CREATE VIEW tr AS
SELECT salestaxrateid AS id,
    *
FROM sales.salestaxrate CREATE VIEW st AS
SELECT territoryid AS id,
    *
FROM sales.salesterritory CREATE VIEW sth AS
SELECT territoryid AS id,
    *
FROM sales.salesterritoryhistory CREATE VIEW sci AS
SELECT shoppingcartitemid AS id,
    *
FROM sales.shoppingcartitem CREATE VIEW so AS
SELECT specialofferid AS id,
    *
FROM sales.specialoffer CREATE VIEW sop AS
SELECT specialofferid AS id,
    *
FROM sales.specialofferproduct CREATE VIEW s AS
SELECT businessentityid AS id,
    *
FROM sales.store;
...
\ pset tuples_only off -- 805 rows in BusinessEntity but not in Person
